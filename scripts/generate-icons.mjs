...
const THEME_DEFAULTS = [
  // fallback aleatorio — solo temas suaves que funcionan en cualquier forma.
  // Los temas agresivos (split/slides/cascade) se asignan por keyword, nunca al azar.
  "breathe", "soft", "glow-parts", "drift", "pulse-parts",
  "wave", "ripple", "shimmer", "breathe", "soft",
];

// Temas por categoría del manifest — la decisión primaria tras THEME_BY_NAME.
const CATEGORY_THEME = {
  ...
  "Mapas": "pin-drop",
  "Bases de datos": "db",
  "Redes sociales": "expand",
  "Emociones": "pulse-parts",
  "Mood": "sway",
  "Celebración": "spark",
  "Deporte": "bounce-flat",
  "Deportes": "shine-parts",
  "Tecnología": "chip",
  "Media": "screen",
  "Abecedario": "breathe",
  "Marcas": "breathe",
...
// Keywords semánticos — ganan sobre categoría (se evalúan en orden).
const KEYWORD_THEMES = [
  [/^(hamburger|pizza|burger|sandwich|hotdog|fries|taco|burrito)/, "bounce-flat"],
  [/^(coin|coins|circle-dollar|circle-euro)/, "spin-y"],
  [/^(globe|earth|planet|atom|disc|record)/, "spin-y"],
  [/^(bell|alarm|alert|warning|vibrate|siren)/, "shake-x"],
  [/^(party|balloon|sparkler|confetti|firework)/, "pop-bounce"],
  [/(clock|watch|timer|stopwatch|hourglass)/, "needle"],
  [/^(arrow-|arrows-)/, "arrow"],
  [/^chevrons?/, "rise-parts"],
  [/^(mail|inbox|mailbox)/, "wave-x"],
  [/^(message|chat|comment)/, "pop"],
  [/^(cloud)/, "drift"],
  [/(rain|drizzle|snow)/, "rain"],
  [/(wind|breeze)/, "wave"],
  [/^(sun|sunset|sunrise|sun-moon)/, "sun-rise"],
  [/^(moon)/, "breathe"],
  [/(heart)/, "ecg"],
  [/^(leaf|tree|flower|sprout|plant)/, "flutter"],
  [/^(bird|fish|bug|cat|dog|rabbit|paw)/, "bounce-flat"],
  [/^(car|bus|truck|train|tram|bike|motorbike|van)/, "vehicle"],
  [/^(plane|airplane|rocket|helicopter)/, "air"],
  [/^(ship|boat|sailboat|anchor)/, "sea"],
  [/^(music|song|audio|sound|volume|headphone|speaker)/, "notes"],
  [/^(mic)/, "amplify"],
  [/^(video|film|movie|camera|webcam)/, "screen"],
  [/^(image|photo|picture)/, "photo"],
  [/^(chart|graph|stats|trending)/, "bars-up"],
  [/^(database|server|hard-drive|hdd)/, "db"],
  [/^(code|terminal|console|braces|brackets|json|xml)/, "split"],
  [/^(git)/, "branch"],
  [/^(lock|key|shield|password|fingerprint)/, "lock-bolt"],
[/^(user|person|users|contact|account)/, "bounce-flat"],
  [/^(file|document|folder|note|clipboard|archive)/, "unfold"],
  [/^(wallet|money|cash|coin|banknote|dollar|euro|bitcoin)/, "coin"],
  [/^(shopping|cart|bag|basket|store|shop)/, "roll"],
  [/^(gift|present)/, "unbox"],
  [/^(trophy|medal|award|crown)/, "shine-parts"],
  [/^(game|dice|sword|controller|joystick)/, "dice"],
  [/^(face|smile|frown|laugh|grin|angry|meh|annoyed)/, "pulse-parts"],
  [/^(wifi|bluetooth|signal|antenna|router|network)/, "signal"],
  [/^(battery|plug|zap|power|charge)/, "charge"],
  [/^(book|library)/, "unfold"],
  [/^(pen|pencil|brush|edit|highlighter|marker)/, "draw-stagger"],
  [/^(scissors|crop|eraser)/, "swing-parts"],
];
...
function buildStart(theme, parts, seedName) {
  ...
  switch (theme) {
    ...
    case "jelly": {
      // squash & stretch orgánico — para formas redondas/comida
      boxes.forEach((b) => {
        push(b.i, {
    ...
    case "float-bob": {
      // flotación relajada con vaivén — para objetos que levitan
      boxes.forEach((b) => {
        const wob = (r(seed + b.i) - 0.5) * 5;
        push(b.i, { y: [0, -3, 0], rotate: [0, wob * 0.4, 0], scale: [1, 1.03, 1] }, 1.4, 0.14 * b.i);
      });
      break;
    }
    case "spin-y": {
      // giro 3D tipo portal — monedas, globes, discos
      boxes.forEach((b) => {
        push(b.i, { rotateY: [0, 360], scale: [1, 0.92, 1] }, 1, 0.06 * b.i);
      });
      break;
    }
    case "pop-bounce": {
      // pop con rebote exagerado — celebración
      boxes.forEach((b) => {
        push(b.i, { scale: [0.6, 1.18, 0.92, 1.08, 0.97, 1], rotate: [0, r(seed + b.i) * 10 - 5, 0] }, 0.8, 0.09 * b.i);
      });
      break;
    }
    case "shake-x": {
      // vibración horizontal corta — alertas, campanas
      boxes.forEach((b) => {
        push(b.i, { x: [0, -1.5, 1.5, -1, 1, -0.5, 0], rotate: [0, -2, 2, -1, 1, 0, 0] }, 0.55, 0.04 * b.i);
      });
      break;
    }
    case "wave-soft": {
      // onda que recorre las partes de abajo hacia arriba
      boxes.forEach((b) => {
        push(b.i, { y: [0, -2.5, 0], x: [0, r(seed + b.i) > 0.5 ? 1 : -1, 0] }, 0.9, 0.16 * b.i);
      });
      break;
    }
    default: {
...
for (const entry of ICON_MANIFEST) {
  ...
  const keywordTheme = KEYWORD_THEMES.find(([re]) => re.test(lucideName))?.[1];
  const categoryTheme = CATEGORY_THEME[entry.category];
  const theme =
    themed ||
    keywordTheme ||
    categoryTheme ||
