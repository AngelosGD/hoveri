              ...
              <div className="group relative overflow-hidden rounded-xl border border-zinc-200 bg-white shadow-sm dark:border-zinc-800 dark:bg-zinc-900">
                ...
                <div className="relative flex items-center justify-between border-b border-zinc-100 bg-zinc-50/50 px-4 py-2.5 backdrop-blur-sm dark:border-zinc-800 dark:bg-zinc-900/50">
                  <div className="flex items-center gap-2">
                    ...
                    <span className="ml-2 font-mono text-xs text-zinc-500 dark:text-zinc-400">
                      {tab === "install" ? `Instalar con ${manager}` : tab === "code" ? "Código del icono" : managerInstall}
                    </span>
                  </div>
                  <motion.button
                    type="button"
                    ...
                    } ${tab === "install" || tab === "code" ? "opacity-100 md:opacity-0 md:group-hover:opacity-100" : "opacity-100"}`}
                  >
                    {copied ? (
                      <>
                ...
                <div className="relative bg-white p-4 dark:bg-zinc-900">
                  ...
                  {tab === "steps" && (
                    <div className="space-y-4">
                      {[
                        { n: "1", title: "Instala la librería", body: managerInstall, mono: true },
                        { n: "2", title: "Importa el icono", body: `import { ${componentName} } from "hoveri";`, mono: true },
                        { n: "3", title: "Úsalo en tu componente", body: `<${componentName} size={${size}} />`, mono: true },
                        { n: "4", title: "Pásale el mouse", body: "Cada icono anima solo. ¿Control total? usa el ref con startAnimation().", mono: false },
                      ].map((s) => (
                        <div key={s.n} className="flex gap-3">
                          <span className="mt-0.5 flex h-5 w-5 shrink-0 items-center justify-center rounded-full bg-rose-500/10 font-mono text-[11px] font-semibold text-rose-500">
                            {s.n}
                          </span>
                          <div className="min-w-0 flex-1">
                            ...
                            {s.mono ? (
                              ...
                              <p className="mt-1 text-xs leading-5 text-zinc-500 dark:text-zinc-400">{s.body}</p>
                            )}
                          </div>
                        </div>
                      ))}
